...
def handler(event, context):
    """
    Lambda function to route queries to the general agent
    """
    try:
        ...
        if not function_name and "inputText" in event:
            ...
            function_name = "invoke_general_agent"
            parameters = {"query": event.get("inputText", "")}
        
        if function_name != "invoke_general_agent":
            return {
                "messageVersion": "1.0",
                "response": {
        ...
        print(f"Error invoking general agent: {e}")
        ...
        return {
            ...
            "response": {
                ...
                "functionResponse": {
                    "responseBody": {
                        "TEXT": {
                            "body": f"Error invoking general agent: {str(e)}"
                        }
                    }
                }
            }
